...
#ifdef ESP32
              size_t data_len = sizeof(serverIndex);
//            AsyncWebServerResponse* response = request->beginResponse(200, "text/html", (const String &) serverIndex, data_len);
...
//            AsyncWebServerResponse* response = request->beginResponse(200, "text/html", serverIndex);
            AsyncWebServerResponse* response = request->beginResponse(LittleFS, "/stylized.html", "text/html");
#else
...
#ifdef ESP8266
                    int command = U_FS;
                    //kk - size_t fsSize = ((size_t)FS_end - (size_t)FS_start);
                    // kk+
                    FSInfo fs_info;
